...
- (NSArray *)specifiers {
    ...
    PSSpecifier *group = [PSSpecifier groupSpecifierWithName:@"RegionShot 0.5.0"];
...
- (void)diagnoseCapture:(BOOL)launch {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 3 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        ...
        NSString *message;
        if (!received) {
            message = @"SpringBoard 在 3 秒内未响应。请确认安装的是 0.5.0、已重新启动 SpringBoard，并检查 RootHide 注入管理器是否允许 RegionShot 注入 SpringBoard。此状态尚不能确认插件已加载。";
        } else {
