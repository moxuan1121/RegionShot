...
补充用户视频参考后，将初始底部工具条与已选区操作条分开显示，已选区操作条跟随矩形并在拖动时隐藏；四角改为 L 形，区域浮图保持选区位置和比例，默认启用双击选区确认，增加状态栏右半侧右滑入口。详见 VIDEO_REFERENCE.md，尚未宣称一比一完成视频效果。

用户已确认 0.3.2 可触发冻结选区面板；未将其扩大为所有入口、裁剪、触摸和稳定性均已验证。

- 接通截图/浮图/OCR/长图采样/AI 功能参数，提供跨进程 AI 配置入口；具体当前功能见 README。
- 新增图片问答、OCR、全屏选择、历史工具条入口；OCR 结果编辑、复制/分享/分词/文字问答/AI 翻译形成完整调用流程。
- 历史采用后台串行写入、UUID 临时目录完整提交、数量/容量限额和中断清理；提供搜索名称/日期、预览缩放、恢复浮图、问答、分享、改名和删除。恢复不重复记录。
- 区域和浮图菜单分别配置，复用同一校验与图标导入逻辑；新增常用系统图标预览、搜索和完整名称选择。
- 提高选区、长图和历史窗口的层级，避免被 RegionShot 已打开的 AI 窗口遮挡。
- 历史不含系统截图、OCR 内容搜索和 AI 对话存档；浮图菜单仍使用系统样式；AI 翻译仍依赖用户配置的 AI 服务。
- 自动滚动、完整三种长图方式、多引擎/多人设、完整编辑器、其他全部详细设置、插件与指令的剩余范围保留，不宣称完成全部复刻。

以下为前序版本报告，旧版本的功能缺项和验证结论按各自版本理解。
